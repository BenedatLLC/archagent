...
| [`usertest/ROUND-1.md`](usertest/ROUND-1.md) | **Design only — no results yet.** The first round to ask whether someone *else* can install the tool and get a result from the documentation alone: the kit ships httpx with no archagent set up in it. Rehearsing it already found a defect on the first screen a new user sees. |
