...
There are **three** tables, not one, and they are never joined. Each has a different unit of observation,
so a column that is meaningful in one is meaningless in the others:

| Table | One row is | Where |
|---|---|---|
| `ledger.csv` | a run of the tool scored against a rubric | §17 below |
| `usertest.csv` | one person's first contact with the tool | §17.2 |
| `defects.csv` | one encounter with a repository archagent had not seen | §17.3 |

`tests/usertest_ledger.py` carries a `refuse_join()` that raises when called, so a future reader reaching
for a combined chart finds the reason rather than the absence of an obstacle: the tables share a 1-5 scale
and no subject.
...
### 17.2 The user-test ledger

One row per person's first contact with the tool (`usertest.csv`, written by `scripts/usertest.py
ingest`). The subject is not an artifact but **whether someone can get a result at all**, so the keys are
different in kind: which documentation they read, whether they had a coding agent, and what they already
knew. `generating_model` is meaningless here and `judge_model` is always human, which is why these rows
cannot live in `ledger.csv`.

**The four dimensions are never averaged**, and `usertest_ledger.py` has no `mean()` — a test asserts it
never grows one. `ease_of_use` is a direct observation of what this design measures; `correctness` is a
spot check whose weight is `claims_verified` and nothing else. Round 1 would read 1.75, which says less
than "1" and "2" do apart.

**`docs_path` is a comparability key, not a footnote.** Round 1's tester could not fetch the pinned
documentation and worked from `--help` and the installed prompts, so that round measured a harder
question than the kit asked. Recorded as `fallback`; `bundled` and `published` are also kept apart,
because rendering, cross-link navigation and in-page search are part of whether documentation is usable
and a kit reader has none of them.

### 17.3 The fresh-target defect ledger

**The question it exists to answer.** Is the tool converging, or will every unseen repository keep
producing a fresh batch of defects? That question was put to the project after 1.0.0 and could not be
answered, because nothing counted. `docs/designs/extraction-confidence.md` records the exchange; this is
the instrument it needs.

One row per **encounter with a repository archagent had not seen before**, generated by
`scripts/defects.py` from the issue tracker. Never typed: of the twelve defects across the first three
encounters, nine were filed as issues and three existed only as commit prose — and those three were the
ones the author found while generating an artifact, which is the category most at risk of going
unrecorded. They are now filed retroactively (#42–#44) with `provenance:
reconstructed-from-commits` on their row.

**The recording requirement.** A defect found while working with a fresh target is filed as an issue and
labelled, *even when the fix lands in the same hour*. The label is the record; a commit message is not.

```
found-by:<encounter-id>        e.g. found-by:usertest-2
```

The issue body states which encounter found it and reproduces the defect against the target at its pinned
...
**Comparability keys**, none of which exist in the other two tables — which is the mechanical reason this
is a third table rather than a column:

| Key | Why |
|---|---|
| `target_fresh` | a repeat encounter is not a fresh-target observation |
| `workflow_reached` | `init` / `describe` / `check` / `evaluate` / `full` — an encounter that stopped at `init` had no chance to find a `drift` defect |
| `observer` | `author` / `independent` / `agent`; an author finds different defects than a stranger |
| `defect_bar` | `verified-and-filed` or `reported-unverified`, so a later round cannot quietly lower the bar and report an improvement |

**The count is never a rate.** `workflow_reached` is ordinal at best, so a denominator would be invented.
Round 1 found 3 defects and round 2 found 6 on the *same repository*; that is a tester who got further,
not a regression, and the keys are what make it readable. Same rule as the user-test dimensions in §17.2:
report the vector, never the mean.

**What it cannot yet say.** Three encounters, two of them fresh, and every comparability key differs
across the rows. This is a record, not a trend, and it carries no information about convergence until
roughly five or six encounters under a stable protocol. Saying so here is the point: the falsification
condition in `extraction-confidence.md` becomes checkable at that size and not before.
