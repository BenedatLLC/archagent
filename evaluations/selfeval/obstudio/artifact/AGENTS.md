...
This repo's architecture is described under `docs/architecture/` and enforced by **archagent**.
...
- Read `docs/architecture/constitution.md` (always) and the relevant `docs/architecture/subsystems/<name>.md`.
- Architectural rules live in `docs/architecture/invariants.md` and are enforced by `archagent check`.
...
- `archagent gen` — regenerate checker configs from `docs/architecture/invariants.md` (`check` does this for you).
